		...
		<table class="table table-striped table-hover">
			...
			@foreach($ProduitsFinis as $produit)
				<tr>
					...
					@else
						<td class="hidden-xs">
							<input type="number" name="quantite_{{$produit->pivot->produit_fini_id}}-{{$produit->pivot->pointure}}_" id="quantiteId_{{$produit->pivot->produit_fini_id}}-{{$produit->pivot->pointure}}_" min="0" max="<?php echo $produit->pivot->quantite ?>" value="<?php echo $produit->pivot->quantite ?>" readonly/>
						</td>
					@endif
